// Andy-Chi
// copy and modify from https://gist.github.com/jserv/4dfaf78cf516cc20f4bc55ce388c195d
...
void prettyPrintTree(struct st_node* node, char* prefix, bool isLeft) {
    char *tt;
    if(node->right) {
        tt = (isLeft ?  "│  ": "   ");
        char temp[300]={0};
        sprintf(temp, "%s%s", prefix, tt);
        prettyPrintTree(node->right, temp, false);
    }

    printf("%s%s", prefix, (isLeft ? "└─ " : "┌─ "));
    // struct treeint *v = treeint_entry(node);
    printf("%d\n", treeint_entry(node)->value);

    if (node->left) {
        tt = (isLeft ? "   " : "│  ");
        char temp1[300]={0};
        sprintf(temp1, "%s%s", prefix, tt);
        prettyPrintTree(node->left, temp1, true);
    }
}
